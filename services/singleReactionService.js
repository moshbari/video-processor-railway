...
class SingleReactionService {
  ...
  async createExtendedMainVideo(mainVideoPath, lastFramePath, mainDuration, totalDuration, outputPath, dimensions) {
    ...
    const frozenCmd = `ffmpeg -loop 1 -i "${lastFramePath}" -f lavfi -i anullsrc=r=48000:cl=stereo -t ${freezeDuration} -c:v libx264 -preset medium -crf 20 -pix_fmt yuv420p -r 30 -c:a aac -ar 48000 -ac 2 -b:a 320k -shortest -y "${frozenPath}"`;
    ...
    const normalizeCmd = `ffmpeg -i "${mainVideoPath}" -vf "scale=${dimensions.width}:${dimensions.height}:force_original_aspect_ratio=decrease,pad=${dimensions.width}:${dimensions.height}:(ow-iw)/2:(oh-ih)/2:black,setsar=1" -r 30 -c:v libx264 -preset medium -crf 20 -pix_fmt yuv420p -c:a aac -ar 48000 -ac 2 -b:a 320k -y "${normalizedMainPath}"`;
  ...
  async createPipVideo(backgroundVideoPath, pipVideoPath, outputPath, pipPosition, dimensions, pipScale = 35, audioSource = 'both', workDir = null) {
    ...
    const cmd = `ffmpeg -i "${backgroundVideoPath}" -i "${pipVideoPath}" -filter_complex "${filterComplex}" -map "[outv]" -map "[outa]" -c:v libx264 -preset medium -crf 20 -c:a aac -ar 48000 -ac 2 -b:a 320k -movflags +faststart -y "${outputPath}"`;
