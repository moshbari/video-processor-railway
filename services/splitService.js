  ...
  async splitVideoForReactions(videoPath, reactions) {
    ...
    try {
      ...
      for (let i = 0; i < sortedReactions.length; i++) {
        ...
        // Skip if duration is too short (less than 0.5s)
        if (duration <= 0.5) {
          console.log(`Skipping short segment: ${currentTime}s to ${clipEndTime}s (${duration.toFixed(2)}s)`);
          
          // Still add to reaction guide
          reactionGuide.push({
