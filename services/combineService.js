...
async function standardizeClipTo30fps(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    const cmd = `ffmpeg -y -i "${inputPath}" -c:v libx264 -preset medium -crf 20 -r 30 -c:a aac -ar 48000 -ac 2 -b:a 320k -pix_fmt yuv420p "${outputPath}"`;
...
async function createBackgroundVideo(framePath, outputPath, duration, width, height, audioSource) {
  return new Promise((resolve, reject) => {
    const cmd = `ffmpeg -y -loop 1 -i "${framePath}" -i "${audioSource}" -t ${duration} -vf "scale=${width}:${height}:force_original_aspect_ratio=decrease,pad=${width}:${height}:(ow-iw)/2:(oh-ih)/2,fps=30" -c:v libx264 -preset medium -crf 20 -c:a aac -b:a 320k -shortest "${outputPath}"`;
    exec(cmd, (error, stdout, stderr) => {
      if (error) {
...
async function overlayPip(bgPath, pipPath, outputPath, coords, targetWidth, targetHeight) {
  return new Promise((resolve, reject) => {
    ...
    const cmd = `ffmpeg -y -i "${bgPath}" -i "${pipPath}" -filter_complex "${filterComplex}" -map "[outv]" -map 0:a -c:v libx264 -preset medium -crf 20 -c:a aac -b:a 320k "${outputPath}"`;
...
async function createPipSegment(originalPath, reactionPath, outputPath, targetWidth, targetHeight, pipPosition) {
  ...
  await new Promise((resolve, reject) => {
    const cmd = `ffmpeg -y -loop 1 -i "${lastFramePath}" -i "${reactionPath}" -t ${reactionDuration} -vf "scale=${targetWidth}:${targetHeight}:force_original_aspect_ratio=decrease,pad=${targetWidth}:${targetHeight}:(ow-iw)/2:(oh-ih)/2,fps=30" -map 0:v -map 1:a -c:v libx264 -preset medium -crf 20 -c:a aac -b:a 320k -shortest "${frozenBgPath}"`;
  ...
  const cmd = `ffmpeg -y -i "${frozenBgPath}" -i "${reactionPath}" -filter_complex "${filterComplex}" -map "[outv]" -map 0:a -c:v libx264 -preset medium -crf 20 -c:a aac -b:a 320k "${outputPath}"`;
