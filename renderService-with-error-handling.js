...
class RenderService {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (let i = 0; i < sortedReactions.length; i++) {
          ...
          // Check if reaction has audio file
          const audioPath = reaction.audioPath || null;
   ...
   * Now supports optional audio file for voice reactions
   */
  createReactionSegment(videoPath, freezeTime, text, duration, outputPath, audioPath = null) {
    return new Promise((resolve, reject) => {
      ...
      if (audioPath) {
        console.log(`Using voice audio: ${audioPath}`);
      }
      ...
      ffmpeg(videoPath)
        ...
        .on('end', () => {
          ...
          // Step 2: Create video with text and audio
          let ffmpegCmd;
          
          if (audioPath && fs.existsSync(audioPath)) {
            // Use provided voice audio
            console.log('Creating reaction with voice audio');
            ffmpegCmd = `ffmpeg -loop 1 -i "${tempImagePath}" -i "${audioPath}" ` +
            ...
            // Use silent audio
            console.log('Creating reaction with silent audio');
            ffmpegCmd = `ffmpeg -loop 1 -i "${tempImagePath}" ` +
